...
class RobotLoop:
    def __init__(self):
        ...
        self.scan_reporter = LoopSkipReporter(
            resumed_decision=DECISION_SCAN_RESUMED,
            # (#scan-flap-2026-09-05) Пять тиков молчания, прежде чем объявлять.
            # Один проход без одобренных — обычное дело: символ проходит отбор
            # сетапа и тут же блокируется своим гейтом, который пишет своё
            # событие. Без задержки лента набирала пару «замолчал → возобновил»
            # каждые два тика и переставала отвечать на вопрос, ради которого
            # писалась: стоим мы или рынок ничего не даёт.
            min_duration_sec=float(getattr(settings, "SCAN_SILENCE_MIN_SEC", 300.0)),
        )
    ...
    async def step(
        ...
        for symbol in bot.config_json.get("symbols", []):
            ...
            # (#tz-enforce-2026-08-03, ревизия 08-04) Условия ТЗ. Считаются
            # всегда; блокируют вход при TZ_MODE=enforce для вооружённых семейств
            # (TZ_ENFORCE_CONDITIONS). Вооружены беспороговые kama,di,obv —
            # структурные фильтры входа (сторона KAMA, направление, объём). adx
            # (порог 23 «и растёт») и stoch (пересечение на баре) в тени: замер
            # дал ADX 16–19 на сетапах, enforce на 23 останавливал трендовый
            # контур целиком. Копим их распределение в тени, калибруем по данным.
            ...
            sig = Signal(
                ...
                plan_json={
                    ...
                    # (#setup-quality-plan-2026-09-04) Разложение оценки сетапа
                    # на слагаемые. Писалось в события интеллекта, но НЕ в план
                    # сделки — поэтому весь разбор стопов по setup_quality.*
                    # молча отбрасывался за отсутствием данных, а выглядело это
                    # как «признак не разделяет группы».
                    #
                    # Важно именно разложение, а не только final_score:
                    # structure_quality считается по шкале «хорошо для лонга» и
                    # для шортов нигде не зеркалится. Пока в журнал попадала
