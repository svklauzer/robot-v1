...
class RobotLoop:
    def __init__(self):
        ...
        self.scan_reporter = LoopSkipReporter(
            ...
            # (#scan-flap-2026-09-05) Молчание объявляется сразу, а снимается
            # только после пяти минут непрерывной работы. Один проход с
            # одобренным символом — не возобновление: символ проходит отбор
            # сетапа и тут же блокируется своим гейтом, который пишет своё
            # событие. Без гистерезиса лента набирала пару «замолчал →
            # возобновил» каждые два тика.
            clear_hold_sec=float(getattr(settings, "SCAN_SILENCE_MIN_SEC", 300.0)),
        )
