...
class Settings(BaseSettings):
    ...
    #
    # ДЕФОЛТ ВКЛЮЧЁН после выгрузки analytics_24h от 2026-07-30. Новая выборка
    # показывает, что минус уже не в одном trend_up_candidate, а в обоих MTF
    # тренд-направлениях (trend_up_candidate −57.28 USDT, trend_down_candidate
    # −51.03 USDT), тогда как CRT/reversal_long остаются около нуля/в плюсе.
    # Значит текущая первопричина течи — не разовый параметр геометрии, а
    # распределение капитала в режимы с отрицательным фактическим ожиданием.
    # Ось не блокирует входы: она ужимает размер до наблюдательного и сама
    # возвращает полный риск, когда rolling expectancy восстановится.
    ...
    # (#capital-leak-2026-07-30) Свежая выгрузка analytics_24h: grade B дал
    # 252 закрытия и -118.20 USDT, тогда как grade A дал +9.52 USDT. Даже
    # усиление RR/confidence внутри B не меняет знак, поэтому B переводится в
    # learning-only до явного включения флагом.
    ENABLE_GRADE_B_TRADING: bool = False
