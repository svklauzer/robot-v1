...
class Settings(BaseSettings):
    ...
    # ==================================================================
    # Пустая строка = автоматический вызов MI-Engine (Market Intelligence).
    # Ручная фиксация: trend_only | range_only | volatile_scalp | "".
    ACTIVE_MARKET_REGIME_OVERRIDE: str = ""
    # Границы переключения контуров на 1h.
    ...
    # Мотив измерен на боевых #264282: win-rate 67% (выше эталонного
    ...
    # Единый флаг включения трендового контура.
    ...
    #
    # 12.08: переводим в enforce, потому что телеметрия #375410 показала
    ...
    #          которого не было: без него зона = lastЧ0.997…1.003);
    ...
    # 2.23.1%, у проигравших 0.000.05%)  требовать цель внутри 1.5Ч от неё
    ...
    # объясняет замер: ТОЧКИ ВХОДА НЕТ. Зона входа задана как lastЧ0.997…1.003,
    ...
    #   убытка). При этом средний MAE −0.618%, а стопы стоят на 13%: типичная
    ...
    # TRX ~0.089% (HTX-native, глубокий). DOT убран  спред 0.21%+ (хронический
    # неликвид). ADA убран по ЖИВОЙ телеметрии: спред 0.140.61% (а не ~0.017%,
    ...
    # НЕ добавлены  их spot спред (0.121%) тоже хуже порога.
    ...
    # MAX_ACTIVE_SIGNALS_PER_SYMBOL=1 Ч 7 символов HTX_SYMBOLS.
    ...
    # осталась в пределах дневного лимита: 5 Ч RISK_PER_TRADE_PCT(1.0) = 5%
    ...
    # экспозиции (капитал Ч плечо)  и теперь это ЕДИНСТВЕННЫЙ действующий.
    ...
    # сделку, свободный капитал, MAX_POSITION_MARGIN_PCT Ч плечо и эта доля.
    ...
    # (#leak-give-back) Был 0.80  но проигравшие шли в плюс лишь на 0.30.66%
    ...
    # 0.45 ловит откатчиков в диапазоне 0.451.2%. Над-килла нет: выход всё равно
    ...
    # закрывается ГАРАНТИРОВАННО в минус (замер 1925.07: round-trip 0.15%,
    ...
    # Замер на 16 реальных траекториях (264282) через боевой ExitPolicyService,
    ...
    # окупает издержки), поэтому в полосе MFE 0.350.53 фиксированный замок
    ...
    # в «чурн-зоне» (меньше churn_max ≈ 2Ч round-trip). Больший сдвиг = рынок
    ...
    # «Широко» = текущий спред > multЧ(EWMA-база символа) ИЛИ > абс. потолка.
    ...
    LIQ_SPREAD_BASELINE_MULT: float = 3.0   # вход: широко если >3Ч базы символа
    ...
    # этом на тех же 1h ADX 2429 И РАСТЁТ, +DI выше −DI на 2335 пунктов,
    # объём 2.94.5Ч, цена выше всех трёх EMA. Тренд есть по каждому прибору,
    ...
    # Старт трейла/фиксации позже → раннеры доезжают к TP2; мелкие плюсы (0.451.3)
    ...
    # Совместимое имя из нового патча. Каноническое поле остаётся CRT_TP2_RR,
    # чтобы не создавать два независимых источника истины для одного параметра.
    CRT_TP2_RENDER_RR: float | None = None                # R:R для TP2 (1:2)
    ...
    # ETH #220)  шорты «от сопротивления» против 1h RSI 7678, т.е. фейд
    ...
    # net_rr_tp2 (runner платит >1.10Ч стопа). Как у тренда: судим по TP2, не по TP1.
    ...
    # цель сайзинга SCALP_MAX_POSITION_MARGIN_PCT (0.20 Ч equity). Два
    ...
    # (все 0.5/0.6-варианты в минусе). Плюс телеметрия: positive_then_negative 5762%
    ...
    # arm = max(SCALP_BREAKEVEN_ARM_PCT, TP1_dist Ч эта доля). Микро-скальп (TP1
    ...
    # Гарантия giveback 0.4: замок фиксирует ≥0.6Чarm_eff ≈ 0.37%+ для range  выше костов.
    ...
    # держим до жёсткого стопа (MIN Ч MULT). Реальный минус/CVD-разворот  закрытие сразу.
    ...
    # (5 ТФ Ч 250 свечей на символ, ~50 запросов к бирже) на КАЖДЫЙ вызов, а
    ...
    TRAJ_MAX_POINTS: int = 400          # при переполнении прореживаем Ч2 и удваиваем шаг
    # --- Post-loss cooldown (только range-скальп) ---
    # После убыточного закрытия по паре символ+сторона не лезем повторно N минут
    #  режет churn (DOT шортился 6Ч в аптренд, пока guard не заблокировал).
    ...
    # POSITION (trend/crt) едет 1.53%: спред  не главный фильтр, но 0.20 был
    ...
    # Телеметрия 27.07: ADA trend_down со setup_score 88100 отклонён ВОСЕМЬ раз
    ...
    OB_POSITION_CVD_THIN_MIN_TRADES: int = 15    # 13 сделки  не выборка
    ...
    # окно даёт 15 сделок и обычный CVD-фильтр не включался. 0 → выкл.
    ...
    # Динамическое плечо по conviction (грейд Ч сила тренда Ч волатильность).
    ...
    # (рынок и чек-лист сетапа). false  прежний храповик max(base, setupЧK),
    ...
    # (#impulse-noise-2026-09-08) БЫЛО 0.0  ростом считалось любое +е.
    ...
    # чего сетапы со setup_score 85100 отклонялись как `symbol_policy_confidence_
    # too_low` (ADA 27.07: 64.4 против требуемых 65.0 при истории в 2 сделки).
    # Это не риск-менеджмент, а шум: на выборке 12 знак результата случаен.
    ...
    # резало целиком. 5 позиций Ч ~13% = 65% < общий потолок 70% → диверсификация.
    ...
    # арбитраж (2 хеджа Ч 10.5% Ч 2 ноги) + 5% сетка ≈ 117% при капитале 950.
    ...
    # хеджи Ч ~2 нотионала  спотовая нога без плеча) и сетку (маржа исполненных
    ...
    # TP1 (массовые b_rr_tp1_too_low по AVAX/BTC/ETH/XRP при net_rr_tp2 2.52.7).
    # Софт-пол 0.10 (только санити против вырожденного TP1≈entry). ЭКОНОМИКУ судит
    # TP2: production_gate min_rr_tp2 (1.301.45, «живой» и в paper) + anti-drain
    ...
    # позиций (5 Ч 1% = 5%): без этого запаса день, в котором весь портфель
    ...
    # (медиана 48 открытий/сутки, максимум 23 в аномальный день 25.06).
    #
    ...
    # занимали 400 из 3000 экспозиции (13%), то есть работающее плечо 1.3Ч при
    # заданном 10Ч. При 1% сделка ~200, пять позиций  33% экспозиции и
    # эффективные 3.3Ч. Выше не пускает MAX_POSITION_MARGIN_PCT=0.13.
    ...
    # и 5Ч123=615 < 665  влезает 5 параллельных (= ANTI_DRAIN_MAX_OPEN_POSITIONS).
    ...
    # как просил Капитан; поставь 0.40.5, если захочешь подушку под добор/2-й вход).
    DYNAMIC_MARGIN_CAP_PCT_OF_FREE: float = 1.0
    # (#sizing-leak-2026-08-12) НЕ делим бюджет на всю вселенную по умолчанию.
    # Телеметрия #375410: при потолке 70% от 950 USDT fair-share давал всего
    # ~95 USDT маржи на символ, затем B-cap 0.5 и regime-sizer 0.4/0.15 сжимали
    # позицию до 1938 USDT нотионала. Это выглядело как «капитал распределён»,
    ...
    # Регулируемое плечо для размера по динамическому бюджету (нотионал = маржаЧплечо).
    ...
    GRID_VOL_MULTIPLIER: float = 1.2            # m_vol: мартингейл объёма (1.11.5)
    GRID_STEP_MULTIPLIER: float = 1.1           # m_step: расширение шага (1.051.2)
    GRID_VOL_COEFF: float = 0.5                 # k_vol: шаг = ATR·k_vol
    # (#audit-grid) Анти-пила флипа: разворот засчитывается только когда цена
    # реально ушла от EMA на kЧATR  почасовое обнимание EMA больше не пилит
    # корзину (12 из 20 последних закрытий были flip-минусами, −1.73 USDT).
    # (#grid-flip-2026-07-09) 0.5→1.0: телеметрия 89 июля  BTC три flip-минуса
    # подряд (−0.58/−0.23/−0.09) при цене в ±0.5ЧATR от EMA200. Полный ATR
    ...
    # spreadЧmult + fee_round, иначе цикл платит спред каждым кругом (кейс
    # AAVE neutral-грид при спреде 0.10.5%).
    ...
    # (#grid-flip-2026-07-09) 0.25→0.60: BTC-чоп 89 июля ходил ±0.5% вокруг
    ...
    # TP-круга и завышал минус всех закрытий (~1.52.5 из −4.05 за 136 циклов
    ...
    # Замер 12 циклов 2526.07 разделяет исходы по RSI без единого исключения:
    ...
    MIN_NET_PNL_TP2_USDT: float = 1.5 # 29.08.2026 ,было 3.5
    ...
    # Причина: TP2 = риск(дистанция до KAMA-стопа) Ч 3.2 давало 612% при
    ...
    # источник positive_then_negative (5762%) и отрицательного роллинг-PnL.
    ...
    # «TP2 превышен» ненаблюдаемо в принципе: сделка, которая прошла бы 3ЧTP2,
    ...
    # ≥ 3.0%  при том, что медианный MFE наших режимов 0.521.19%, а типичный
    # стоп 0.60.9%. Ни одна не может взвестись на обычной сделке: после TP1 у
    # позиции ровно два исхода  дошла до TP2 или сползла на безубыток.
    # Замер на боевых (0203.09.2026): ETH #470 MFE 0.86%→+0.26%, BTC #459
    ...
    # ~100125 USDT не пропускал его вовсе (#476, #483, #495, #497), а
    ...
    # эта смесь платила минимум MIN_NET_RR_BLENDED Ч |стоп|. Заменяет фиктивную
    ...
    #      вселенной лежит в 2543, то есть условие лонга не выполнялось почти
    ...
    # у 25% сделок (замер по 16 траекториям сигналов 264282).
    ...
    # полоса [0.533, 0.80) оказалась пустой. Три сделки с MFE 0.430.49 ушли в
    ...
    # MFE в выборке 264282 = 1.535%, ни одна сделка не дотягивала до 1.80 →
    ...
    # 130665 USDT защищаемый нетто = 0.33…1.66 USDT  гейт 2.50 блокировал
    ...
    # 100 USDT это 0.250.60 USDT завышения за сделку.
    ...
    # теоретическим: доля 10.5% Ч 2 хеджа Ч ~2 капитала на нотионал давала ~42%
    ...
    def production_blockers(self) -> list[str]:
        blockers: list[str] = []
        # --- Конфигурационные sanity-checks патча ---
        allowed_regimes = {"", "trend_only", "range_only", "volatile_scalp"}
        if self.ACTIVE_MARKET_REGIME_OVERRIDE not in allowed_regimes:
            blockers.append(
                "ACTIVE_MARKET_REGIME_OVERRIDE must be one of: "
                "'', 'trend_only', 'range_only', 'volatile_scalp'"
            )
        if self.REGIME_CHOP_ADX_MAX >= self.REGIME_TREND_ADX_MIN:
            blockers.append(
                "REGIME_CHOP_ADX_MAX must be lower than REGIME_TREND_ADX_MIN"
            )
        if self.REGIME_MIN_HISTORY_FOR_CALIB < 1:
            blockers.append("REGIME_MIN_HISTORY_FOR_CALIB must be >= 1")
        envelope_total = (
            float(self.CAPITAL_ENVELOPE_DIRECTIONAL_PCT)
            + float(self.CAPITAL_ENVELOPE_ARB_PCT)
            + float(self.CAPITAL_ENVELOPE_GRID_PCT)
        )
        if abs(envelope_total - 95.0) > 1e-9:
            blockers.append(
                "capital envelopes must sum to exactly 95% "
                f"(currently {envelope_total:g}%)"
            )
        if self.CRT_TP2_RENDER_RR is not None and abs(
            float(self.CRT_TP2_RENDER_RR) - float(self.CRT_TP2_RR)
        ) > 1e-9:
            blockers.append(
                "CRT_TP2_RENDER_RR conflicts with canonical CRT_TP2_RR; "
                "configure only one value"
            )
        ...
        # Типовой нотионал = эквити Ч MAX_POSITION_MARGIN_PCT (плечо 1).
    ...
    # 0.20 USDT на позицию 100, а carry за время удержания набирает 0.0060.18.
    ...
    # её уже нет. При нотионале $100Ч2 даже успешный круг давал бы центы.
    #
    ...
    # ставка выходила 210% от той, по которой входили (XRP 21.93% → 0.51%),
    # окупаемость комиссий уезжала на 40150 дней при лимите удержания 14,
    ...
    # ОДНОМУ шумному снапшоту (−0.60 = 3Ч комиссии), спред возвращался через
