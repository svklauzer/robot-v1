...
class Settings(BaseSettings):
    ...
    # TP_REACH_MODE: shadow | enforce. В enforce не берём сделки, где TP1
    # дальше фактически достижимого хода режима/символа; это сохраняет сделки
    # с работающей геометрией, а не просто душит объём.
    ...
    # Запасная цель, когда структуры в коридоре нет. Используется ТОЛЬКО если
    # замер типового хода недоступен (мало сделок) — иначе TP1 ставится по
    # измеренному MFE, см. TP1_TYPICAL_MOVE_MULT.
    TP1_DEFAULT_PCT: float = 1.2
    # (#tp1-from-measured-move-2026-08-22) Доля ИЗМЕРЕННОГО типового хода для
    # запасной TP1. 1.0 = медианный MFE режима: половина сделок до цели доходит,
    # что для точки ЧАСТИЧНОЙ фиксации (50%, остаток едет на TP2) и есть
    # определение достижимости.
    #
    # Зачем не константа: в аптренде `resistance` (максимум последних баров)
    # остаётся позади цены и в коридор [+0.6%; +1.8%] не попадает, поэтому
    # «запасной» вариант был основным. Константа 1.2% при медианном ходе
    # 0.49–0.54% давала отношение 2.2–3.1 при потолке гейта 1.5 — система
    # ставила цель, которую сама же признавала недостижимой, и резала вход.
    TP1_TYPICAL_MOVE_MULT: float = 1.0
