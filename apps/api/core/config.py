...
class Settings(BaseSettings):
    ...
    # Увеличиваем пороги чтобы меньше сделок блокировалось рано
    FAILED_SETUP_MFE_SOFT_PCT: float = 0.35
    FAILED_SETUP_MFE_MID_PCT: float = 0.65
    FAILED_SETUP_MFE_DEEP_PCT: float = 0.90

    # Пороги убытка для принудительного закрытия слабого setup до TP1.
    # Делаем менее агрессивными чтобы давать сделкам больше времени
    FAILED_SETUP_LOSS_SOFT_PCT: float = -0.40
    FAILED_SETUP_LOSS_MID_PCT: float = -0.60
    FAILED_SETUP_LOSS_DEEP_PCT: float = -0.85

    # MFE-протекция и частичная фиксация в процентах.
    # Сдвигаем вверх чтобы защищать прибыль позже
    PROTECTIVE_MFE_START_PCT: float = 0.65
    PROTECTIVE_DRAWDOWN_SHARE: float = 0.70
    ADAPTIVE_TRAIL_MFE_START_PCT: float = 1.50
    ADAPTIVE_TRAIL_DRAWDOWN_PCT: float = 0.65
    ...
    # Снижаем минимальные требования к net PnL чтобы больше сделок проходило
    MIN_NET_PNL_TP1_USDT: float = 1.5
    MIN_NET_PNL_TP2_USDT: float = 3.0
    ...
    # Уменьшаем множитель стопа чтобы ставить ближе и снижать риск
    LEVELS_STOP_ATR_MULT: float = 1.5
    LEVELS_MIN_STOP_PCT: float = 0.30

    # Дополнительные фильтры качества setup в learning/paper.
    # Снижаем требования чтобы пропускать больше качественных сетапов
    LEARNING_SETUP_MIN_SCORE: float = 55.0
    LEARNING_SETUP_MIN_TREND_ALIGNMENT: float = 40.0
    LEARNING_SETUP_MIN_VOLUME_CONFIRMATION: float = 5.0

    # Минимальная защищаемая прибыль для exit-политики
    # Увеличиваем чтобы не фиксировать прибыль слишком рано
    MIN_PROTECTIVE_EXIT_PCT: float = 0.35
    MIN_POST_TP1_EXIT_PCT: float = 0.50
    MIN_PROTECTIVE_NET_USDT: float = 0.35
    MIN_PROTECTIVE_R_MULT: float = 0.10
