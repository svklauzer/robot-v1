...
def _params(side="sell", reduce_only=False, hedged=False, market_type="swap", margin_mode="isolated"):
    return LiveExecutor.order_params(client_id="trendopen0123", market_type=market_type, side=side,
...
def test_htx_leverage_body_has_no_foreign_keys(htx_ccxt, direct_retry):
    ...
    assert sent == [{"lever_rate": 1, "contract_code": "XRP-USDT", "margin_mode": "isolated"}]
...
# ── путь live-ордера ───────────────────────────────────────────────────────────
class _Client:
    def __init__(self, *, hedged=False, leverage_error=None, mode_error=None, create_error=None):
        self.hedged = hedged
...
def test_position_mode_failure_keeps_the_last_known_mode(monkeypatch):
    ...
    ex._position_mode = (True, 0.0)          # кеш устарел
...
def test_rejected_order_forgets_the_cached_position_mode(monkeypatch):
    ...
    assert not res.ok
    assert ex._position_mode is None
