...
def system_config_effective():
    """Действующее значение каждой настройки и откуда оно взято.

    (#three-places-2026-09-07) Мест НЕ два, а три: дефолт в config.py, значение
    в render.yaml и значение в дашборде Render. Первые два раньше и назывались
    здесь — но `source` их не различает и различить не может: в окружении
    процесса запись из блупринта и запись из дашборда неотличимы.

    Разница между ними при этом существенная: ключ с `value:` в блупринте
    возвращается синком к записанному, ключ с `sync: false` или отсутствующий в
    блупринте — нет. Поэтому `source: env` читается как «перекрыто окружением»,
    а не как «перекрыто render.yaml»: второе — догадка, и один раз она уже
    стоила времени (OKX_MARKET_TYPE не было ни там, ни там).
    """
