...
def analytics_stop_forensics(window_hours: float = 720.0, regime: str | None = None,
    ...
    `outcome=tp1` (#tp1-forensics-2026-09-11) — те же признаки входа, но группы
    делятся по достижению TP1. Проверка перед гейтом на TP1: отбирает ли хоть
    что-то на входе сделки, дошедшие до TP1, или гейт стал бы выключателем
    торговли, как гейт по TP2.
    """
    from services.stop_loss_forensics import _OUTCOMES, build
    ...
    try:
        return build(db, window_hours=window_hours, regime=regime, side=side,
                     max_rows=max_rows, outcome=outcome)
    finally:
        db.close()
    ...
    try:
        ...
        for s in signals:
            ...
            rows.append({
                ...
                "side": s.side,
                # (#entry-mode-quality-2026-09-19) Способ входа как ось разбора.
                # Замер форы показал, что рыночные входы теряют, а перенесённые
                # зарабатывают, — но по одному PnL нельзя отличить «вход хуже по
        ...
        by_regime: dict = {}
        by_entry_mode: dict = {}
        by_entry_mode_regime: dict = {}
        for r in rows:
            by_symbol_regime.setdefault(f"{r['symbol']}|{r['regime']}", []).append(r)
            by_regime.setdefault(r["regime"], []).append(r)
            by_entry_mode.setdefault(r["entry_mode"], []).append(r)
            by_entry_mode_regime.setdefault(f"{r['entry_mode']}|{r['regime']}", []).append(r)
        ...
        entry_mode_items = []
        for mode, bucket in by_entry_mode.items():
        ...
        return {
            ...
            "by_entry_mode": entry_mode_items,
            "by_entry_mode_regime": entry_mode_regime_items,
            "daily": daily[-30:],
        }
    finally:
        db.close()
