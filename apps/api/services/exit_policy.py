...
class ExitPolicyService:
    ...
    def before_tp1_decision(
        ...
        # (#be-lock-preempts-ride-2026-08-26) В ТРЕНДЕ замок вооружается позже.
        #
        # `BREAKEVEN_LOCK_ARM_PCT=0.35` калибровался под скальп — в самом конфиге
        # написано «СНИЖЕНО с 0.45 — ловим скальпы 0.25%+». К трендовой сделке он
        # применялся тем же числом, и получалось так: замок стоит ВЫШЕ по коду,
        # чем блок ride, поэтому срабатывал первым и делал ride недостижимым.
        # `TREND_RIDE_MIN_MFE_TO_PROTECT_PCT=0.8` при этом лежал без дела.
        #
        # Замер 24–26.08: шесть победителей закрыты `breakeven_stop`, и каждый
        # оставил рынку больше, чем забрал:
        #
        #   TRX #433  MFE 0.71%  держали 38 ч → +0.79 USDT
        ...
        #
        # capture_rate по всей выборке 7.95%, у trend_up_candidate −10.1%.
        # Тезис берётся с 4h (один ATR там 1.6–3.2%), а фиксировались мы внутри
        # трети первого бара. Сидели в засаде сутками и спрыгивали через доллар.
        #
        # Ни одной новой константы: берём настройку, которая ровно это и значит
        # — «минимальный MFE, после которого трендовую сделку защищаем». Скальп
        # и range не затронуты, у них замок остаётся прежним.
        if is_trend_mode:
