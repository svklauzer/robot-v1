...
_TERMINAL_REASONS: frozenset[str] = frozenset({
    ...
    "post_tp1_lock_stop",  # стоп остатка на уровне после TP1 — объявлен на TP1
})
...
class TPReach:
    ...
    # (#shadow-verdict-2026-09-06) Вердикт гейта НЕЗАВИСИМО от режима. В shadow
    # `allowed` всегда True, а `reason` — «mode_shadow»: собственное решение
    # гейта в записи не оставалось совсем.
    #
    # 06.09 режим переведён в shadow ради проверки «прав ли гейт»: он судит по
    # достижению TP2, тогда как сделка платится через TP1 и трейл (замер 04.09:
    # дошедшая до TP1 даёт +0.98R против −0.83R у недошедшей). Ответить на это
    # можно, только сравнив исходы сделок, которые гейт пропустил бы, с теми,
    # что остановил бы. Без записанного вердикта сравнение пришлось бы выводить
    # заново из чисел — повторяя логику гейта по памяти, вместе с ветвью
    # спасения по нецензурированной частоте.
...
def evaluate(*, symbol: str, regime: str, tp1_dist_pct: float | None,
    ...
    return TPReach(
        ...
        # После ветви спасения: это заключение гейта, а не то, что ему позволил
        # режим.
        would_block=not within,
        reason=(
