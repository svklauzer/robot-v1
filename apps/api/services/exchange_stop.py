...
позицию свопа:
  ...
  • полное закрытие снимает стопы робота по символу;
  • ручные стопы и ордера владельца робот не видит и не трогает: свои он
    узнаёт по префиксу номера клиента (services/robot_orders.py).
...
class ExchangeStopService:
    ...
    async def cancel_all(self, db, signal, *, alert: Alert | None = None,
        ...
        бы по чужой цене. Поэтому снимаются все стопы РОБОТА по символу (кроме
        записанных за другими открытыми сделками), а не только записанный, и
        неудача доходит до владельца. Ручные стопы владельца робот не видит и
        не трогает (#manual-orders-2026-09-16).
        """
        try:
            mode = stops_mode(self.executor)
            if mode is None:
