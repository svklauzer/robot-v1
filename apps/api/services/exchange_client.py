...
class ExchangeClient(Protocol):
    ...
    def set_swap_leverage(
        self, symbol: str, leverage: float, margin_mode: str, position_side: str | None = None
    ): ...

    # (#live-margin-posmode-2026-09-16) Режим счёта, формат номера ордера и имя
    # параметра стороны позиции у бирж разные — каждый клиент знает свои.
    POSITION_SIDE_PARAM: str

    def fetch_derivatives_account(self) -> dict: ...

    def make_client_order_id(self, purpose: str) -> str: ...

    # (#exchange-stop-2026-09-16) Стоп-лосс на бирже: страховка на время, когда
    # робот не работает (рестарт, деплой, OOM).
