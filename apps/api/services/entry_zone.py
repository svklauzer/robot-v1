...
def depth_thinness(side: str, snapshot: dict | None, levels: int = 5) -> float | None:
    ...
    (#depth-levels-2026-09-08) ЗАВИСИТ ОТ ГЛУБИНЫ ФИДА, и это не оттенок.
    Числитель — первые `levels` уровней, знаменатель — вся видимая книга. Если
    фид отдаёт ровно `levels` уровней, метрика тождественно равна 1.0 и
    перестаёт быть метрикой.

    Так и вышло 07.09, когда фид перевели на OKX `books5` (ровно пять уровней):
    `near_depth_share` стал ровно 1.0 у каждого сигнала, порог
    ENTRY_ZONE_MIN_NEAR_DEPTH_SHARE=0.25 перестал срабатывать, и КАЖДЫЙ вход
    пошёл по рынку вместо переноса к микро-VWAP или стенке. Заодно wall_share
    (max/total по 10 уровням) вырос с 0.2–0.6 до 0.7–1.0, а obi по пяти уровням
    стал бить в veto 0.8 втрое чаще.

    Вывод для следующего раза: смена площадки фида — это не только вопрос
    сравнимости записанных признаков, но и вопрос живых порогов, которые эту
    книгу читают. Прежде чем менять фид, проверить, что глубины хватает ВСЕМ
    потребителям: здесь, в OrderBookAnalyzer.wall_share и в imbalance.
    """
    if not snapshot:
        return None
    book = _levels(snapshot.get("bids") if str(side).lower() == "long" else snapshot.get("asks"))
