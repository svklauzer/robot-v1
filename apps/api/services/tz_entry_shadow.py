...
CONDITION_FAMILY = {
    ...
    "adx_below_min": "adx",
    # (#tz-enforce-adx-rising-2026-09-03) «ADX растёт» вынесен в СОБСТВЕННОЕ
    # семейство и больше не делит его с `adx_below_min`. Это два разных по
    # природе условия, и склеенные они были невключаемы:
    #   * adx_below_min — АБСОЛЮТНЫЙ порог TZ_ADX_MIN, написанный под
    #     DEX-таймфреймы и на наших не откалиброванный (см. докстринг модуля);
    #   * adx_not_rising — ПРОИЗВОДНАЯ, порога не требует вовсе: сравнивает ADX
    #     с самим собой на предыдущем баре. Работает на любых таймфреймах, как
    #     di/obv.
    # Пока они были одним семейством, включить проверку производной можно было
    # только вместе с некалиброванным порогом — то есть практически никогда.
    #
    # Замер на боевой выборке 01–03.09.2026 (15 трендовых сделок HTX+OKX,
    # выборка маленькая — это сигнал, а не доказательство):
    #   провалившие adx_not_rising: 11 сделок, 2 прибыльных, сумма −9.74 USDT
    #   не провалившие:              4 сделки, 4 прибыльных, сумма +8.90 USDT
    # Все крупные убытки — вход в тренд, который на входе УЖЕ затухал.
    "adx_not_rising": "adx_rising",
    "di_against_side": "di",
