...
def substitutes_adx_rising(latch_snapshot: dict) -> bool:
    ...
    (#latch-kind-2026-09-05) Только импульс ПО ADX. Первая версия принимала
    любой живой импульс, и в ленте 05.09 нашлось прямое подтверждение, что это
    неверно: у ADA защёлка стояла на кроссе Stoch при adx_delta −0.87, то есть
    в enforce сняла бы отказ `adx_rising` там, где ADX падал.

    Подменять условие можно только его собственным событием. Кросс Stoch —
    другое условие, и вдобавок `stoch` не входит в вооружённые семейства
    (`kama,di,obv,adx_rising`): теневой сигнал снимал бы enforce-блокировку.
    Он по-прежнему записывается — как наблюдение, а не как основание.

    И только в enforce: направление, сторона KAMA и объём остаются нетронутыми,
    защёлка утверждает «импульс ADX был недавно», а не «всё прочее в порядке».
    """
    if mode() != "enforce" or not latch_snapshot.get("live"):
        return False
    impulse = latch_snapshot.get("impulse") or {}
    return str(impulse.get("kind") or "") == IMPULSE_ADX_TURN
