...
class MarketIntelligenceEngine:
    ...
    def _fallback_tp1_pct(self, regime: str | None) -> float:
        ...
        (#tp1-from-measured-move-2026-08-22 — ОТМЕНЕНО 24.08.2026)

        Правка 22.08 ставила эту цель по измеренной медиане MFE — тем же числом,
        которым гейт достижимости цель ПРОВЕРЯЛ, и обосновывала это как «одно
        число вместо двух спорящих». Обоснование неверное: выводить одно из
        другого правильно для тождеств учёта (маржа, конверты), но не для пары
        «цель ↔ её валидация». Проверка, выведенная из проверяемого, ничего не
        проверяет — отношение становилось 1.0 по построению.

        Хуже того, вместе с полом коридора это давало безусловный замок:
        `TP1_MIN_PCT=0.6` при потолке отношения 1.5 означал, что гейт проходим
        только при медиане ≥ 0.4%. У ETH медиана 0.3154% — блокировка каждый
        скан, а медиана растёт только от закрытых сделок, которых нет.

        Возвращаем константу. Причина, по которой от неё уходили (гейт резал
        вход с отношением 2.2–3.1), исчезла вместе с самим отношением: гейт
        считает ЧАСТОТУ достижения TP2 и сравнивает с порогом из заявленного RR,
        см. `services/tp_reachability.py`.
        """
        return float(getattr(settings, "TP1_DEFAULT_PCT", 1.2))
