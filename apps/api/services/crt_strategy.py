...
class CRTStrategyService:
    def evaluate(self, htf_candles, ltf_candles, *, symbol: str | None = None,
        ...
        if mode == "either" and not (mss or fvg):
            return None
        # (#mss-is-not-a-confirmation-2026-09-20) "fvg" — вход только при FVG;
        # MSS перестаёт быть самостоятельным основанием, оставаясь бонусом к
        # оценке. Замер недели: пять сделок с MSS дали −17.26 USDT, убыточны
        # четыре из пяти, а худший бакет «только MSS» — три сделки на −12.84
        # (−1.72% на номинал) против −0.14% у одиннадцати сделок с одним FVG.
        if mode == "fvg" and not fvg:
            return None
