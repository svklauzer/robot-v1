...
//
// (#three-places-2026-09-07) Настройка живёт в ТРЁХ местах: дефолт в config.py,
// значение в render.yaml и значение в дашборде Render. Страница раньше называла
// два и подписывала env как «перекрыто render.yaml» — а это догадка: в
// окружении процесса запись из блупринта и запись из дашборда неотличимы.
//
// Разница между ними существенная: ключ с `value:` в блупринте синк возвращает
// к записанному, ключ с `sync: false` или отсутствующий — нет. Совет «поправь
// render.yaml» на дашбордном ключе не сработает, и один раз это уже стоило
// времени: OKX_MARKET_TYPE не было ни в блупринте, ни в дашборде, и решение
// принимал питоновский дефолт.
//
      ...
      {data && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <Card
              label="Задано в окружении"
              value={data.from_env}
              hint="render.yaml ИЛИ дашборд Render — по значению не отличить; правка config.py не подействует"
              accent="text-sky-400"
            />
            <Card
              ...
              {data.removable_env_keys?.length > 0 && (
                <div className="rounded-xl border border-amber-700/40 bg-amber-950/20 p-4">
                  ...
                  <p className="text-xs text-neutral-400 mt-1">
                    ...
                    Удаление ничего не меняет в поведении, только уменьшает шум. Сначала
                    посмотрите, ГДЕ ключ задан: в блупринте его может не быть вовсе —
                    тогда чистить нечего, значение пришло из дашборда.
                  </p>
                  <div className="text-xs text-neutral-500 mt-2 font-mono break-all">
