...
- Есть Telegram routing, menu service, `/subscription_status` alias, delivery log/worker, product E2E smoke и owner test endpoints.
...
3. Поддерживать E2E-smoke `POST /system/product-e2e-smoke` для создания payment event и выдачи/продления subscription.
...
- E2E smoke Telegram/payment/subscription перед релизом.
...
6. Запускать и расширять Telegram/payment/subscription E2E smoke (`POST /system/product-e2e-smoke`).
